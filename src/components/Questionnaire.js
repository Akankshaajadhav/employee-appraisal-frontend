...
let horizontal='center'
  ...
  const handleSave = async () => {
    ...
    if (question_type === "MCQ" || question_type === "Single_Choice") {
      ...
      if (options.length === 0) {
        setSnackbar({
          open: true,
          message: "At least one option is required for MCQ or Single Choice questions.",
          severity: "error",
        });
        return;
      }
  ...
  return (
    ...
    <Box sx={{ display: 'flex', justifyContent: 'flex-start', fontSize: '20px', color: '#3b7dda',ml:'20px', mt:'20px' }}>
      ...
      {/* Split Screen Layout */}
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, height: '100vh', gap: 2 }}>
        {/* Left Panel */}
        ...
        {/* Right Panel */}
        <Card sx={{ p: 1, width: "40%" ,flex: 1,display: "flex", flexDirection: "column" }}>
          <CardContent>
        <Box sx={{  display: 'flex', flexDirection: 'column', p: 2, minWidth:300, maxHeight:"500px" }}>

          {/* Content */}
          <Box sx={{flex:1}}>
            <Box sx={{display:'flex',justifyContent:'flex-start', mb: 2, color:'#3b7dda'}}>
             ...
             {/* MCQ (Conditional Rendering) */}
                        {question_type === "MCQ" && (
                          <div className='mcq_setup'>
                            ...
                            <button type="button" onClick={handleAddMcqOption}>+</button>
                        </div>
              )}
                          ...
                          {question_type === "Yes/No" && (
                            <div className='mcq_setup'>
                              ...
                              <div className='input_option'>
                                <TextField
                                  value={yesNoLabels[0]}
                                  onChange={(e) => handleYesNoLabelChange(0, e.target.value)}
                                  variant="standard"
                                />
                              </div>
                              <div className='input_option'>
                                <TextField
                                  value={yesNoLabels[1]}
                                  onChange={(e) => handleYesNoLabelChange(1, e.target.value)}
                                  variant="standard"
                                />
                              </div>
                            </div>
          ...
          <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 1, mt: "auto" ,mb:3,ml:2}}>
